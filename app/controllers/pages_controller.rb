class PagesController < ApplicationController
	...
	def user_index
		...
		@joined_communities.each do |community|
			rooms = Community.joins(:room).where('rooms.community_id = ? AND rooms.isFinished =\'f\'', community.id).select("communities.name as community_name, communities.id, rooms.name")
			rooms.each do |room|
